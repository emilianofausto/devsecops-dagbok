...
class DiaryEntryBase(BaseModel):
    ...
    title: str = Field(
        ..., min_length=1, max_length=100, description="Title cannot be empty"
    )
    content: str = Field(..., min_length=1, description="Content cannot be empty")
    category: str = Field(
        ..., min_length=1, max_length=50, description="Category cannot be empty"
    )

class DiaryEntryCreate(DiaryEntryBase):
    """Schema for creating a new diary entry. (user_id intentionally omitted)"""
